...
const Cart = () => {
    ...
    return (
        <>
            <h1 className='text-center text-5xl mb-8 bg-black p-10 text-white font-serif font-thin'>Your Bag</h1>
                    ...
                    products.map((pro) => (
                        <div key={pro.id} className="flex justify-between gap-4 p-4 border rounded-lg shadow-md w-full max-w-lg">
                            ...
                            <div className='flex flex-col'>
                                ...
                                <button onClick={() => removeProduct(pro.id)} className='mt-2 bg-red-500 text-white px-4 py-2 rounded'>Remove</button>
                            </div>
                            <div className='flex flex-col items-center justify-center'>
                                <button onClick={() => incrementCount(pro.id)} className='bg-green-300 rounded p-2 text-[20px]'>+</button>
                                <span>{pro.count}</span>
                                <button onClick={() => decrementCount(pro.id)} className='bg-pink-300 rounded p-2.5 text-[20px'>-</button>
                            </div>
                        </div>
                    ))
                }
            ...
            <div className='flex justify-center'>
                <button onClick={clearCart} className='mt-2 bg-pink-500 text-white px-4 py-2 rounded'>Clear</button>
            </div>
        </>
    );
}
